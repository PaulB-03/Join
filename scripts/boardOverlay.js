...
function inertParts(){ return [document.querySelector("header"),byId("sidebar"),document.querySelector(".app")].filter(Boolean); }
...
function onEditTask(id,task,overlay){
  closeOverlay(overlay); if(typeof openTaskOverlay!=="function") return;
  openTaskOverlay(); byId("taskOverlay")?.classList.add("edit-mode");
...
function hideTaskOverlay(){
  ...
  closeOverlay(ov); typeof clearTask==="function"&&clearTask();
}
...
function openTaskOverlay(){
  const ov=byId("taskOverlay"); if(!ov) return;
  ov.classList.remove("edit-mode"); showOverlay(ov,{focus:false});
  byId("add")?.removeAttribute("data-editing-id");
  setOverlayButtonText(false); clearTask(); initTaskFormEnhancements();
...
function fillTaskFormFromExisting(id,task){
  ...
  selectedCategory=task.category||"Task"; selectedState=task.state||"toDo";
...
function setPrioFromTask(task){
  resetPrioSelection(); if(task.priority) setPrioColor(task.priority);
}
...
function setOverlayButtonText(isEditing){
  const btn=byId("add"); if(!btn) return;
  btn.textContent="";
...
async function saveSubtasksFromOverlay(taskId){
  ...
  const subs=items.map(el=>{const chk=el.querySelector('input[type="checkbox"]');const txt=(el.querySelector(".txt")?.textContent||"").trim();return{ text:txt,done:!!chk?.checked };});
  const res=await fetch(`${RTDB_BASE}tasks/${taskId}/subtasks.json`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(subs)});
...
function updateSubtaskCountersUI(taskId,done,total){
  ...
  const card=document.querySelector(`.card[data-id="${taskId}"]`)||null; if(!card) return;
  const meta=card.querySelector(".meta span"); if(meta) meta.textContent=`${done}/${total} Subtasks`;
  const bar=card.querySelector(".progress .bar"); if(bar&&total>0) bar.style.width=`${Math.round((done/total)*100)}%`;
}
...
async function onDetailSubtaskChange(e){
  ...
  const detail=cb.closest(".task-detail"), taskId=detail?.getAttribute("data-id"); if(!taskId) return;
  ...
  try{
    ...
    console.error(err); cb.checked=previous; alert("Konnte Subtask nicht speichern. Bitte später erneut versuchen.");
  }
}
...
function fillAssignedContacts(contacts){
  ...
  contacts.forEach((c,i)=>{ const div=document.createElement("div"); div.className="av"; div.style.background=window.color(i); div.textContent=initials(c); wrap.appendChild(div); });
}

function fillSubtasks(subtasks){
  const wrap=document.querySelector(".addedSubtaskWrapper"); wrap.innerHTML="";
  (subtasks||[]).forEach(s=>{ let txt=typeof s==="string"?s:s.text; txt=txt.replace(/^•\s*/,""); wrap.appendChild(createSubtaskElement(txt,!!(typeof s==="object"&&s.done))); });
}
...
function renderAssignedInitials(){
  ...
  assignedContacts.forEach((c,i)=>{ const d=document.createElement("div"); d.className="av"; d.style.background=window.color(i); d.textContent=initials(c); w.appendChild(d); });
}
...
function mountDatePickerMinToday(){
  ...
  const tryOpen=()=>{ try{ const ua=navigator.userActivation; const ok=ua&&(ua.isActive||ua.hasBeenActive); if(typeof input.showPicker==="function"&&ok) input.showPicker(); }catch{} };
