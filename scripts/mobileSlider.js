function initDropzoneCarousel(dropzone) {
  ...
  dropzone.addEventListener("pointerup", (e) => {
    // geändert von pointerdown zu pointerup, damit sich die cards im Overlay wieder öffnen, doppelte addEventListener Belegung
    isDown = true;
  ...
  ["pointerup", "pointercancel", "pointerleave"].forEach((evt) => {
    dropzone.addEventListener(evt, (e) => {
      ...
      isDown = false;
      try {
        dropzone.releasePointerCapture(e.pointerId);
      } catch (_) {}
  ...
  dropzone.addEventListener("keydown", (e) => {
    ...
    if (e.key === "ArrowRight") {
      e.preventDefault();
      dropzone.scrollBy({ left: step, behavior: "smooth" });
    }
    if (e.key === "ArrowLeft") {
      e.preventDefault();
      dropzone.scrollBy({ left: -step, behavior: "smooth" });
