...
function setPrioColor(index) {
  ...
  Array.from(prioRefs).forEach((element) => element.classList.remove("removeHoverEffect"));
...
function addBackgroundColor(prioRef, prioImg) {
  prioRef.classList.add(prioRef.id === "urgent" ? "redColor" : prioRef.id === "medium" ? "orangeColor" : "greenColor");
...
function addPrioImgColor(prioRef, prioImg) {
  prioRef.classList.add("removeHoverEffect");
  prioImg.classList.add("filterWhite");
}

function removePrioImgColor(prioRef, prioImg) {
  prioRef.classList.remove("removeHoverEffect");
