...
function setPrioColor(index) {
  ...
  const ref = refs[index], img = ref?.querySelector("img");
...
function showAddedToastAndRedirect() {
  ...
  setTimeout(() => { msg.remove(); overlay.remove(); location.href = "board.html"; }, 1600);
}
...
function getSubtasksFromForm() {
  ...
  if (!wrapper) return [];
  return Array.from(wrapper.querySelectorAll(".subtask, .subtaskTitle")).map(el => {
